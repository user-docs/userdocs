defmodule Userdocs.Projects do
  ...
  def list_projects(%{access_token: access_token, context: %{repo: Client}} = opts) do
    ...
    {:ok, %{body: body}} = HTTPoison.get("http://localhost:4000/api/projects?#{params}", [{"authorization", access_token}])
    {:ok, %{"data" => projects_attrs}} = Jason.decode(body)
    Enum.map(projects_attrs, fn(attrs) ->
