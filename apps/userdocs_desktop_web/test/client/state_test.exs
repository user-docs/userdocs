...
"""
    test "Loads Initial State", %{remote_project: project, step: step} do
      assert Client.load() == %{
        annotation_types: 1,
        annotations: 0,
    ...
    test "Loads State Based on remote Project ID", %{remote_project: project, step: step} do
      assert Client.load() == correct_object_counts()
    end

    test "Loads State Based on local Project ID", %{local_project: project, step: step} do
      assert Client.load() == correct_object_counts()
    end
    """
    test "thing", %{user: user, remote_project: project, step: step} do
      Client.load()
