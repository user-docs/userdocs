defmodule UserdocsDesktopWeb.ScreenshotLive.ReviewComponent do
  ...
  def assign_default_tab(socket, %Screenshot{status: :size_difference}),
    do: assign(socket, :tab, :side_by_side)

  def assign_default_tab(socket, %Screenshot{status: :difference}),
    do: assign(socket, :tab, :swipe)
  ...
  def screenshot_link(screenshot) do
    Client.current_team()
    |> case do
      %Team{type: team_type} when team_type in [:team, :enterprise] ->
        screenshot.presigned_urls.image.get

      %Team{type: team_type} when team_type in [:personal] ->
        "#{UserdocsDesktopWeb.Endpoint.url()}/images/#{screenshot.id}/image.png?#{NaiveDateTime.utc_now()}"
    end
  end

  def provisional_link(screenshot) do
    Client.current_team()
    |> case do
      %Team{type: team_type} when team_type in [:team, :enterprise] ->
        screenshot.presigned_urls.provisional.get

      %Team{type: team_type} when team_type in [:personal] ->
        "#{UserdocsDesktopWeb.Endpoint.url()}/images/#{screenshot.id}/provisional.png?#{NaiveDateTime.utc_now()}"
    end
  end

  def diff_link(screenshot) do
    Client.current_team()
    |> case do
      %Team{type: team_type} when team_type in [:team, :enterprise] ->
        screenshot.presigned_urls.diff.get

      %Team{type: team_type} when team_type in [:personal] ->
        "#{UserdocsDesktopWeb.Endpoint.url()}/images/#{screenshot.id}/diff.png?#{NaiveDateTime.utc_now()}"
    end
  end
end
