defmodule UserdocsDesktop.BrowserController do
  ...
  def handle_call({:open_browser, css}, _from, %{server: server, page_pid: page_pid} = state) do
  ...
  @impl true
  def handle_cast(:quit, state), do: Kernel.exit(:normal)
  ...
  def cast_step(%Step{step_type_id: "full_screen_screenshot", page_id: page_id, screenshot_id: screenshot_id} = step, _context) do
    {:full_screen_screenshot, %{page_id: page_id, screenshot_id: screenshot_id}}
  end
  def cast_step(%Step{step_type_id: "element_screenshot", margin_left: ml, margin_right: mr,  element: element} = step,
  ...
  def cast_step(%Step{step_type_id: "apply_annotation", annotation: annotation} = step, _context) do
    {:create_annotation, %{annotation: annotation}}
  end
  def cast_step(%Step{step_type_id: "fill_field", element: %{selector: selector, strategy_id: strategy}, text: text} = step, _context) do
    {:fill_field, %{selector: selector, strategy: strategy, text: text}}
  end
  def cast_step(%Step{step_type_id: "click", element: %{selector: selector, strategy_id: strategy}} = step, _context) do
  ...
  def handle_return_value({:ok, result, step}), do: {:ok, result}
  def handle_return_value({:error, message, step}), do: {:error, message}
  def handle_return_value({:warn, message, step}), do: {:warn, message}
