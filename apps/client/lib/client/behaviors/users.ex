defmodule Client.Users do
  ...
  @callback update_user(User, map(), map()) :: {:ok, User} | {:error, Ecto.Changeset}
   ...
   def update_user(user, attrs, state),
    do: Module.concat(impl(state), "Users").update_user(user, attrs, local_or_remote_opts(state))


  def load_users(state, opts) do
    users = Module.concat(impl(state), "Users").list_users(local_or_remote_opts(state, opts))
