defmodule Client.Screenshots.Repo.S3 do
  ...
  def update_screenshot(%Screenshot{base64: base64, presigned_urls: urls} = screenshot, opts \\ %{}) do
    magick_path = Map.get(opts, :magick_path, Paths.imagemagick_executable_path())

    with provisional_binary <- Base.decode64!(base64),
         {:ok, image_binary} <- PresignedUrls.get_object(urls.image.get),
         ...
         {:ok, _} <- PresignedUrls.put_object(urls.provisional.put, provisional_binary),
         {:ok, _} <- PresignedUrls.put_object(urls.diff.put, diff_binary) do
